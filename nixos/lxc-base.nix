{
  ...
  users.users.marcin = {
    ...
    extraGroups = ["wheel"];
    shell = pkgs.fish;
    # Lingering so marcin's systemd --user services (the claude-monitor tailer)
    # run without an active login on these headless CTs. Harmless on the monitor
    # box (no tailer service there).
    linger = true;
